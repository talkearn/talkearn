...
class UserController {
    ...
    async bookUser(req, res) {
        try {
            const {comment, comunitatePreference, email, guestId, name, phone, trigger, age} = req.body

            if (!name) {
                await User.findOneAndUpdate(
                    {_id: guestId, logged: false, type: 'client'},
                    {$set: {email, lead: true}}, {new: true}
                )
            } else {
                await User.findOneAndUpdate(
                    {_id: guestId, logged: false, type: 'client'},
                    {$set: {name, email, phone, comment, comunitatePreference, trigger, age, lead: true}}, {new: true}
                )
            }
